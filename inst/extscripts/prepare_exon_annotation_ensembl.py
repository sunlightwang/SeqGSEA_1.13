#!/usr/bin/python
# to prepare exon annotations from Ensembl gene annotation gtf file
...
for l in aggregates.values():
   for i in xrange( len(l)-1 ):
      assert l[i].name == l[i+1].name, str(l[i]) + str(l[i+1]) + " has wrong name"
      assert l[i].iv.end <= l[i+1].iv.start, str(l[i]) + str(l[i+1]) + " starts too early"
